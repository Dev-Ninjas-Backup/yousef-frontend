      ...
      <div className="p-4">
        <div className="flex gap-2 mb-2">
          <Badge className="bg-white text-gray-700 hover:bg-white border border-gray-300 shadow-none">
            {String(product.condition || "unknown").charAt(0).toUpperCase() + String(product.condition || "unknown").slice(1)}
          </Badge>
          <Badge className="bg-white text-gray-700 hover:bg-white border border-gray-300 shadow-none">
        ...
        <p className="text-2xl font-bold text-blue-600 mb-3">${Number(product.price || 0).toFixed(2)}</p>
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
          <Store className="h-4 w-4" />
          <span>Sold by: {String(product.seller?.name || "Unknown Seller")}</span>
        </div>
